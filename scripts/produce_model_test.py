...
h5ad_to_process = snakemake.input.blood_h5ad
test = snakemake.output.test_inp
...
def preprocess_data(annotated_object: sc.AnnData, columns_to_add: list) -> pd.DataFrame:
    ...
    expression_with_added_cols[cols_to_apply_zscore].T.to_csv("to_deconvolute_test.csv", sep='\t')
    grouped = expression_with_added_cols.groupby("Project")
    expression_with_added_cols[cols_to_apply_zscore] = np.log2(expression_with_added_cols[cols_to_apply_zscore] + 1)    
    
    expression_with_added_cols[cols_to_apply_zscore] = (
    grouped[cols_to_apply_zscore]
    .transform(lambda x: zscore(x, ddof=1))
    .fillna(0))
    expression_with_added_cols.to_csv(test, sep='\t', index_label="Sample")
